...
from cas.localbo_cat import CASMOPOLITANCat
from cas.localbo_mixed import CASMOPOLITANMixed
from cas.localbo_utils import from_unit_cube, latin_hypercube, to_unit_cube, ordinal2onehot, onehot2ordinal
import torch
import logging
from cas.optimizer import *
...
class MixedOptimizer(Optimizer):
    ...
    def restart(self):
        from cas.localbo_utils import train_gp
        ...
        if self.guided_restart and len(self.casmopolitan._fX):
            ...
            # Use deterministic seed generation strategy to ensure reproducibility
            # Generate deterministic seed based on restart count
            if hasattr(self, '_restart_count'):
                self._restart_count += 1
            else:
                self._restart_count = 0
            seed = 42 + self._restart_count * 1000  # Fixed seed base
        ...
        if self.init_design=="best" and self.source:
            top_5a_indices = self.source.y['y'].nlargest(10 * self.casmopolitan.n_init).index
            random_a_indices = np.random.choice(top_5a_indices, size=self.casmopolitan.n_init, replace=False)
            self.X_init = self.source.x.loc[random_a_indices].to_numpy()
        elif self.init_design=="non_zero" and self.source:
            # DAR problem: randomly select n_init points from data with yield > 3
            is_dar = hasattr(self.source, '__class__') and 'DAR' in self.source.__class__.__name__
            
            if is_dar:
                high_yield_indices = self.source.y[self.source.y['y'] > 3].index
                
                if len(high_yield_indices) < self.casmopolitan.n_init:
                    print(f"Warning: Only {len(high_yield_indices)} data points with yield > 3, less than required {self.casmopolitan.n_init}")
                    print(f"Will use all {len(high_yield_indices)} data points with yield > 3")
                    selected_indices = high_yield_indices
                else:
                    # Randomly select n_init points (without changing global random seed)
                    selected_indices = np.random.choice(high_yield_indices, size=self.casmopolitan.n_init, replace=False)
                
                self.X_init = self.source.x.loc[selected_indices].to_numpy()
                print(f"DAR problem (non_zero): Randomly selected {len(selected_indices)} initial points from data with yield > 3")
