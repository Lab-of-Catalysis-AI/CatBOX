...
from cocabo.BaseBO import BaseBO
...
class CoCaBO_Base(BaseBO):
    ...
    def runTrials(self, budget, n_trial):
        ...
        df = self.runOptim(budget=budget, seed=n_trial)
    ...
    def update_weights_for_all_cat_var(self, Gt_ht_list, ht_batch_list, Wc_list, gamma_list,
        ...
        for j in range(len(self.C_list)):
            ...
            if batch_size > 1:
                ...
                Gt_ht = Gt_ht_list[j]
                ht = ht_batch_list[j]  # ht_batch_list is actually ht_list for batch_size=1
                if ht is None or ht >= len(probabilityDistribution):
                    # Fallback: choose a random valid index
                    ht = np.random.choice(len(probabilityDistribution))
                    print(f"Warning: Invalid ht {ht_batch_list[j]}, using fallback {ht}")
